...
# create a line:
my_line <-
	matrix(c(-84, 35, -78, 35), nrow=2, byrow=T) %>%
	st_linestring() %>% # create linestring of class XY
	st_sfc(crs = st_crs("+proj=longlat +datum=WGS84 +no_defs")) %>% # start with WGS84 lon-lat coords
	st_as_sf() %>% # convert sfc -> sf object**
	st_transform(crs=st_crs(nc))

# Simple subset using square brackets:
nc_sub <- nc[my_line,]

# second type of subset using st_join
nc_join <- st_join(nc, my_line, join = st_intersects) %>% suppressWarnings() # doesn't work for some reason...

# Another alternative: get only intersections of line and polygon
nc_intersect <- st_intersection(my_line, nc) %>% suppressWarnings()

# simple subsetting way:
plot(st_geometry(nc), col = "white", border = "grey")
plot(st_geometry(nc_join), add=TRUE, border = "black")
plot(st_geometry(nc_sub), add=TRUE, col = "pink")
plot(st_geometry(my_line), add=TRUE, col = "red")
plot(st_geometry(nc_intersect), add=TRUE, col = "blue")


```


2. Repeat this after setting `sf_use_s2(FALSE)`, and _compute_ the difference (hint: use `setdiff`), and colour the counties of the difference using colour '#88000088'.

3. Plot the two different lines in a single plot; note that R will plot a straight line always straight in the projection currently used; `st_segmentize` can be used to add points on a straight line, or on a great circle for ellipsoidal coordinates.

4. NDVI, normalised differenced vegetation index, is computed as `(NIR-R)/(NIR+R)`, with NIR the near infrared and R the red band.  Read the `L7_ETMs.tif` file into object `x`, and distribute the band dimensions over attributes by `split(x, "band")`. Then, add attribute NDVI to this object by using an expression that uses the NIR (band 4) and R (band 3) attributes directly.
...
8. Using a vector representation of the raster `L7_ETMs`, plot the
